...
from file_util import file_util_get_files, file_util_read_text, file_util_is_ext
...
class MDXContentProcessor:
    ...
    def get_definition_mdx(self, word: str, builder) -> List[bytes]:
        """
        ...
        try:
            ...
            return [f"<h1>Error processing word: {word}</h1>".encode('utf-8')]
            
    def _process_content_links(self, content: List[str], builder) -> List[str]:
            ...
            for file_path in injection_files:
                if file_util_is_ext(file_path, 'html'):
                    try:
                        ...
                        logger.warning(f"Failed to read injection file {file_path}: {e}")
    ...
    def get_definition_mdd(self, path: str, builder) -> List[bytes]:
        """
        ...
        try:
            ...
            if content:
                ...
                logger.debug(f"MDD resource not found: {path}")
                return []
...
def get_definition_mdx(word: str, builder) -> List[bytes]:
    """Backward compatible function for MDX lookup."""
    return _processor.get_definition_mdx(word, builder)

def get_definition_mdd(path: str, builder) -> List[bytes]:
